...
export default function TopCanales({ show, onClose, datos, id = 'TopCanales-' }: TopCanalesProps) {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const chartInstanceRef = useRef<Chart | null>(null);

  useEffect(() => {
    if (!datos?.data || !datos?.options || !canvasRef.current) return;
  ...
  if (!datos?.data || !datos?.options) {
    return <div className="text-sm text-gray-500 italic">No hay datos disponibles para Top Canales.</div>;
  }

  const canvas = (
    <>
      <h3 className="text-lg font-semibold mb-4">Top 10 Canales con más fallas</h3>
