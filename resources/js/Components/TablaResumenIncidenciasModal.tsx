...
export default function TablaResumenIncidenciasModal({ show, onClose, datos }: Props) {
      ...
      <Dialog as="div" className="relative z-50" onClose={onClose}>
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4 overflow-y-auto">
          <DialogPanel className="w-full max-w-4xl transform rounded-xl bg-white p-6 shadow-xl transition-all">
            <h3 className="text-xl font-bold mb-4">🚨 Incidencias registradas</h3>
                ...
                <tbody>
                  {datos.map((fila, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      <td className="border px-4 py-2">{fila.incidencia || 'Sin nombre'}</td>
